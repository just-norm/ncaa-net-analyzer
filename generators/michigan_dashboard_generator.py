#!/usr/bin/env python3
"""
Generate HTML dashboard from Michigan's schedule analysis CSV
"""
...
def generate_html_dashboard():
    ...
    # Read the CSV data
    games = []
    with open('michigan_schedule_analysis.csv', 'r') as f:
    ...
    location_breakdown = {}
    with open('michigan_location_breakdown.csv', 'r') as f:
        reader = csv.DictReader(f)
        for row in reader:
            quad = row['Quadrant']
    ...
    # Read Michigan's own rankings
    michigan_rankings = {}
    michigan_avg = 'N/A'
    try:
        with open('michigan_own_rankings.csv', 'r') as f:
            reader = csv.reader(f)
...
'''
    ...
    def generate_upcoming_rows():
        ...
        rows_html = '''                    <tr style="background: #f0f8ff; border-top: 3px solid #00274C;">
                        <td colspan="7" style="text-align: center; font-weight: bold; color: #00274C; padding: 10px;">
                            UPCOMING GAMES
                        </td>
                    </tr>
'''
...
'''
    ...
    # Build the full HTML
    # Build Michigan's rankings display (NET and AP only)
    rankings_display = []
    for system in ['NET', 'AP']:
        rank = michigan_rankings.get(system, 'NR')
        if rank != 'NR':
...
<head>
    ...
    <title>Michigan Basketball - NET Resume Analysis</title>
    <style>
        * {
            margin: 0;
            padding: 0;
    ...
    <div class="container">
        <div class="header">
            ...
            <div style="margin-top: 15px; font-size: 1.1em;">
                Record: <strong>24-1</strong> (14-1 Big Ten)
            </div>
...
'''
    ...
    html_content += '''                </tbody>
        ...
        <div class="footer">
            ...
            <p style="margin-top: 8px;">Michigan Maize (#FFCB05) and Blue (#00274C)</p>
        </div>
    </div>

    <script>
        console.log('Michigan NET Resume Analyzer loaded successfully');
    </script>
</body>
</html>'''

    # Write to file
    with open('michigan_dashboard.html', 'w') as f:
        f.write(html_content)

    print("✅ Dashboard generated successfully!")
    print(f"   - 10 Q1 wins (Avg NET: {sum(quad_stats['Q1'])/len(quad_stats['Q1']):.1f})")
    print(f"   - 8 Q2 wins (Avg NET: {sum(quad_stats['Q2'])/len(quad_stats['Q2']):.1f})")
